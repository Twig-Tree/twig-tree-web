...
- `api/types.ts`: 백엔드 요청 DTO와 응답 DTO를 분리해서 정의한다. 각 ID를 필수로 둘지, optional로 둘지, `number | null`로 둘지는 실제 API 엔드포인트 스펙을 따른다 — 도메인을 가리지 않고 일괄 적용하지 않는다. 생성·수정 요청 DTO는 엔드포인트 스펙에 없는 ID 필드를 넣지 않는다 — 응답 타입을 그대로 재사용해 불필요한 ID를 요구하지 않는다.
- `model/types.ts`: 프론트 도메인 모델. ID 타입은 대응하는 응답 DTO의 nullability를 그대로 따른다.
