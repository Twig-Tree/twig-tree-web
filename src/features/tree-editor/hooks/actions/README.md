...
- `useAddNodeAction`의 call-site `onError`: editor store를 undo로 복구한다.

노드 삭제처럼 서버 응답만으로 최종 cache 상태를 확정하기 어렵다면 `onSettled`에서 `invalidateQueries`로 서버 상태와 다시 동기화한다.

## 새 Action Hook 추가 체크리스트

`useDeleteNodeAction` 같은 새 action hook을 추가할 때는 다음을 확인한다.
...
`useAddNodeAction`은 임시 editor node/edge를 만들고 `useAddNodeMutation`을 호출한다.
...
`useDeleteNodeAction`은 선택된 노드와 하위 노드 id를 수집한 뒤 editor store에서 먼저 제거하고 `useDeleteNodeMutation`을 호출한다.
