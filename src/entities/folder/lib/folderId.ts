...
인자 : string | null folderId -> 프론트엔드 폴더 ID. 자기 ID처럼 null일 수 없는 값도 그대로 넘길 수 있다
반환값 : 백엔드에 보낼 폴더 ID. null이면 그대로 null
*/
export const getApiFolderId = (folderId: string | null): number | null =>
  folderId === null ? null : Number(folderId);
