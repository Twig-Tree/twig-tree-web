...
- #61 계획의 3단계에 목록 영역 상태 분기를 `isPending` 기준으로 명시했다.
- 로딩·에러 상태에서도 상위 이동 경로를 남기고, 확인 버튼만 비활성으로 두는 것으로 정리했다.
- 팝업의 경로 상태를 `useState` 목록이 아니라 `{ id, name }[]` 스택으로만 들고, 목록 데이터는 전적으로 query가 소유하도록 2단계 hook의 책임을 좁혔다.
...
- #61 계획 4단계를 "트리거와 모달을 함께 가진 컴포넌트가 `isOpen`을 소유한다"로 명시했다.
- `Modal`은 열림 상태를 스스로 갖지 않고 `isOpen`을 prop으로 받는 형태로 만들었다. 상태의 소유자를 호출부가 정할 수 있어야 위 승격이 가능하기 때문이다.
- `Modal`이 닫혀 있을 때 children을 렌더링하지 않도록 해, 팝업이 열려 있는 동안에만 폴더 목록 query가 실행되게 했다.
